...
<form class="cart" method="post" enctype='multipart/form-data'>
    ...
    <div class="detail-group ">
        ...
        foreach ( $grouped_products as $grouped_product ) {
            ...
            ?>
            <ul id="product-<?php the_ID(); ?>" <?php post_class('list-none list-detail-group border radius'); ?>>
                <li class="add_to_cart_quantity_group">
                    <?php if ( ! $grouped_product->is_purchasable() || $grouped_product->has_options() ) : ?>
                        <?php woocommerce_template_loop_add_to_cart(array('style'=>'')); ?>
                ...
                <li>
                    <h3 for="product-<?php echo esc_attr($grouped_product->get_id()); ?>" class="product-title">
                        <?php if($grouped_product->is_visible()) echo '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>'; else echo esc_attr($grouped_product->get_name()); ?>
                    </h3>
                </li>
                <?php do_action( 'woocommerce_grouped_product_list_before_price', $grouped_product );
                $html_price = $grouped_product->get_price_html();
                $stock_html = wc_get_stock_html( $grouped_product );
                if(!empty($html_price) || !empty($stock_html)){ ?>
                <li>
